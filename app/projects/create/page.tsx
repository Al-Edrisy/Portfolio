...
export default function CreateProjectPage() {
  ...
  useEffect(() => {
    ...
    if (!isDevelopment && (user.role as string) !== 'developer' && (user.role as string) !== 'admin') {
  ...
  // Allow admins and specifically whitelisted developer accounts (if role existed)
  const isDevelopment = process.env.NODE_ENV === 'development'
  // @ts-ignore - Role might be extended in future
  if (!isDevelopment && (user.role as string) !== 'developer' && user.role !== 'admin') {
    return (
      <main className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <Navigation />
        <div className="pt-20">
